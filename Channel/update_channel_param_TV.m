function p  = update_channel_param_TV(q)
% =========================================================================
% -- Function to update the required time-variant THz channel parameters
% =========================================================================

% -- Function: p  = update_channel_param_TV()
...
%% Parameters of the Saleh-Valenzuela (S-V) model for multipath (MP)
...
% Define the time of arrival (ToA) for clusters and rays
...
if strcmp(p.ToAType,'Exponential')
    ...
    if (p.Fc + p.BW/2) < 0.55e12
        ...
        if p.d_tx_rx > p.c/p.clusterArrivalRate
            % The adopted cluster arrival rate is verified by measurements
            % for short-distance indoor scenarios. Here we adjust the
            % cluster arrival rate if it results in cluster arrival
            % times shorter than the LoS propagation delay over larger distances.

            p.clusterArrivalRate = p.c/(p.d_tx_rx+(p.c*1e-9/0.13)); % Modified Cluster arrival rate (nsec^-1)
            disp('Warning: The cluster arrival rate is not verified by measurements for the selected communication range. Select shorter distances for accurate results.');
        end
...
%% Spherical and planar wave models

% Supported combinations in this version are (SA/AE): Plane/Plane, Sphere/Plane, Sphere/Sphere (NO steering vector)
...
if isfield(q,'TCApproxMode')
    p.TCApproxMode = q.TCApproxMode;
else
...
if isfield(q,'DopplerSpecShape')
    p.DopplerSpecShape = q.DopplerSpecShape;
else
    p.DopplerSpecShape = 'Jakes';   % Options are: 'Jakes', 'Flat'
end
if isfield(q,'nSamplesperFrame')
    p.nSamplesperFrame = q.nSamplesperFrame;
else
