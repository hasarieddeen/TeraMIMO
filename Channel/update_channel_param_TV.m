...
if isfield(q,'Nsubb')
    p.Nsubb = q.Nsubb;   % Number of subcarriers to divide the total Bandwidth (K-subcarriers)  
else
    p.Nsubb = 1;
end
p.BW_sub = p.BW/p.Nsubb; % Subcarrier bandwidth (Hz)
if isfield(q,'Nsubc')
    p.Nsubc = q.Nsubc;   % Number of sub-bands in each subcarrier
else
    p.Nsubc = 2^9;
end
...
if (p.Fc + p.BW/2) < 0.55e12
    if isfield(q,'clusterDecayFactor')
        p.clusterDecayFactor = q.clusterDecayFactor;       % Cluster arrival decay factor (nsec)
    else
        p.clusterDecayFactor = 3.12*1e-9;
    end
    if isfield(q,'rayDecayFactor') 
        p.rayDecayFactor = q.rayDecayFactor;               % Ray arrival decay factor (nsec)
    else
