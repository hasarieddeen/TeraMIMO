...
Ts = 1/p.BW_sub;
figure,
cmap = colormap(parula(size(H,1)*size(H,2)));
close;
    ...
    for tx_ind = 1: size(H,2)
        for subb_ind = 1: size(H,3)
            ...
            plot(f,mag2db(squeeze(HH)),...
                'Color',cmap((rx_ind-1)*size(H,1)+tx_ind,:));hold on;
            ...
            plot(f,rad2deg(squeeze(HH_ph)),...
                'Color',cmap((rx_ind-1)*size(H,1)+tx_ind,:),'LineStyle','-.','Marker','o');hold on;
            ...
            stem(t_vec1, squeeze(mag2db(abs(h1))), ...
                'Color',cmap((rx_ind-1)*size(H,1)+tx_ind,:),...
                'LineStyle','-.','Marker','o');
            hold on;
